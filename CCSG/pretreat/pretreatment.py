...
replace_type = [tokenizer.Null, tokenizer.String, tokenizer.Identifier, tokenizer.Boolean, tokenizer.FloatingPoint,
                tokenizer.Integer, tokenizer.DecimalInteger]
...
main_type_in = [myJavalang.tree.IfStatement, myJavalang.tree.ForStatement, myJavalang.tree.MethodInvocation,
                myJavalang.tree.FormalParameter, myJavalang.tree.CatchClauseParameter,
                myJavalang.tree.SuperMethodInvocation,
...
def parse(s, mask_part, between):
    ...
    for idx, i in enumerate(tks):
        ...
        if (not check_instace(i, replace_type)) and (not check_instace(i, not_replace_type)) and (
        not isinstance(i, tokenizer.Keyword)):
            print("wait")
...
def normal_parse(text_full, replacement, mask_text=".MASK()", tuple_id=-1):
    try:
        ...
        if mask_text == ".MASK()":
            ...
            return normal_parse(text_full, replacement, ";", tuple_id)
